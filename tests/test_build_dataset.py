...
def _install_fake_hub(monkeypatch, stage_dir: Path) -> _FakeHfApi:
    ...
    hub = SimpleNamespace(HfApi=lambda: api, hf_hub_download=api.fake_hf_hub_download)
...
def _documents_split(stage_dir: Path, split: str) -> pd.DataFrame:
    f = stage_dir / "parquet" / "documents" / split / f"{split}-00000-of-00001.parquet"
...
def test_stage_only_is_the_default(tmp_path, monkeypatch):
    ...
    assert (stage_dir / "parquet" / "windows" / "train").exists()
...
def test_stage_only_never_touches_the_hub(tmp_path, monkeypatch):
    ...
    hub = SimpleNamespace(HfApi=lambda: (_ for _ in ()).throw(
        AssertionError("HfApi must not be constructed on --stage-only")))
    monkeypatch.setitem(sys.modules, "huggingface_hub", hub)
...
def test_no_windows_flag(tmp_path, monkeypatch):
    ...
    assert not (stage_dir / "parquet" / "windows").exists()
    assert len(_documents_split(stage_dir, "test")) == 1
...
def test_publish_detects_hub_mismatch(tmp_path, monkeypatch, capsys):
    ...
    _patch_stage(monkeypatch, tmp_path)
    hub = SimpleNamespace(
        HfApi=lambda: _FakeHfApi(tmp_path / "stage"),
        hf_hub_download=lambda repo_id, filename, repo_type, revision: _corrupt(
            tmp_path, filename),
    )
    monkeypatch.setitem(sys.modules, "huggingface_hub", hub)
